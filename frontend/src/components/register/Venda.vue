<template>
    <el-dialog title="Venda" :visible.sync="modal" width="80%">
        <el-form :model="formulario">
            ...
            <associado :visible.sync="associadoVisible" :pessoa="formulario.pessoaJuridica" @saved="formulario.etapa='Consolidada'"></associado>
        </el-form>
        <span slot="footer" class="dialog-footer">
            <el-button @click="modal = false">Cancelar</el-button>
            <span v-if="formulario.hasOwnProperty('id') && formulario.id != ''">
            <el-button type="primary" v-if="formulario.etapa != 'Fechamento' && formulario.etapa != 'Consolidada'" @click="save">Avançar</el-button>
            <el-button type="warning" v-if="formulario.etapa == 'Fechamento'" @click="associadoVisible = true">Registrar Associação</el-button>
            <el-button type="danger" v-if="formulario.etapa == 'Consolidada'" @click="save">Finalizar</el-button>
            
            </span>
            <span v-if="!formulario.hasOwnProperty('id')">
            <el-button type="success" v-if="formulario.etapa != 'Fechamento' && formulario.etapa != 'Consolidada'" @click="create">Iniciar Venda</el-button>
            <el-button type="warning" v-if="formulario.etapa == 'Fechamento'" @click="associadoVisible = true">Registrar Associação</el-button>
            <el-button type="danger" v-if="formulario.etapa == 'Consolidada'" @click="create">Finalizar</el-button>
            </span>
        </span>
    </el-dialog>
</template>
...
export default {
  ...
  methods: {
    save() {
      ...
      this.$request
        .put(`venda/${this.formulario.id}`, this.formulario)
        .then(response => {
    ...
    init() {
      ...
      this.interesses = this.vendaModel.interesses.map(
        interesse => interesse.descricao
      );
    }
  },
  mounted() {
    this.$request.get("servico").then(response => {
      this.servicos = response.data;
    });

    this.$request.get("etapa").then(response => {
