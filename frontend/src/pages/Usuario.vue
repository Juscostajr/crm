<template>
    ...
    <el-card class="box-card">
        ...
        <cadastrar-usuario :visible.sync="dialogFormVisible" :datamodel="dataModel"></cadastrar-usuario>
...
export default {
    ...
    methods: {
        ...
        find() {
            this.$request.get(`usuario/${this.search}`)
                .then(response => {
                    this.tableData = response.data;
                })
        },
        handleDelete(id) {
            this.$confirm('Você tem certeza que desja excluir este usuário?')
                .then(_ => {
                    this.$request.delete(`usuario/${id}`)
                        .then(response => {
                            this.$notify({
                                type: 'success',
                                title: 'Sucesso',
