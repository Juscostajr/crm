<template>
    <el-card class="box-card">
        ...
        <tipo-grupo :visible.sync="dialogFormVisible" :datamodel="dataModel" @saved="findAll"/>
...
<script>
import TipoGrupo from '../components/register/TipoGrupo.vue';
export default {
    data() {
        return {
            title: 'Tipo de Grupo',
        ...
        findAll() {
            this.$request.get('tipogrupo')
                ...
                .catch(error => {
                    ...
                    this.$notify.error({
                        title: 'Erro!',
                        message: 'Não foi possível carregar os tipos de grupos'
                    })
                })
        },
        find() {
            this.$request.get(`tipogrupo/${this.search}`)
                .then(response => {
                    this.tableData = response.data;
                })
        },
        handleDelete(id) {
            this.$confirm('Você tem certeza que desja este tipo grupo?')
                .then(_ => {
                    this.$request.delete(`tipogrupo/${id}`)
                        .then(response => {
                            this.$notify({
                                type: 'success',
                                title: 'Sucesso',
