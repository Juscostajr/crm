<template>
    ...
    <el-card class="box-card">
        ...
        <el-table :data="tableData" style="width: 100%" empty-text="Nenhum resultado encontrado.">
            ...
            <el-table-column prop="nome" label="Nome" width="260"></el-table-column>
        ...
        <cadastrar-operadora :visible.sync="dialogFormVisible" :datamodel="dataModel"></cadastrar-operadora>
...
<script>
import CadastrarOperadora from '../components/register/Operadora.vue';
export default {
    data() {
        return {
            title: 'Operadoras',
    ...
    methods: {
        ...
        find() {
            this.$request.get(`operadora/${this.search}`)
                .then(response => {
                    this.tableData = response.data;
                })
        },
        handleDelete(id) {
            this.$confirm('Você tem certeza que desja excluir esta operadora?')
                .then(_ => {
                    this.$request.delete(`operadora/${id}`)
                        .then(response => {
                            this.$notify({
                                type: 'success',
                                title: 'Sucesso',
