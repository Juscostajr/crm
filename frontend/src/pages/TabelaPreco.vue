...
export default {
    ...
    methods: {
        ...
        find() {
            this.$request.get(`tabelapreco/${this.search}`)
                .then(response => {
                    this.tableData = response.data;
                })
        },
        handleDelete(id) {
            this.$confirm('Você tem certeza que desja excluir esta tabela de preços?')
                .then(_ => {
                    this.$request.delete(`tabelapreco/${id}`)
                        .then(response => {
                            this.$notify({
                                type: 'success',
                                title: 'Sucesso',
