<template>
    ...
    <el-card class="box-card">
        ...
        <cadastrar-perfil :visible.sync="dialogFormVisible" :datamodel="dataModel"></cadastrar-perfil>
...
<script>
import CadastrarPerfil from '../components/register/Perfil.vue';
export default {
    data() {
        return {
            title: 'Perfils',
    ...
    methods: {
        ...
        find() {
            this.$request.get(`perfil/${this.search}`)
                .then(response => {
                    this.tableData = response.data;
                })
        },
        handleDelete(id) {
            this.$confirm('Você tem certeza que desja excluir este perfil?')
                .then(_ => {
                    this.$request.delete(`perfil/${id}`)
                        .then(response => {
                            this.$notify({
                                type: 'success',
                                title: 'Sucesso',
